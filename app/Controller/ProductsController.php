...
class ProductsController extends AppController {
	...
	public $uses = array('Product','ProductCategory', 'ProductAssignOption', 'ProductGallery', 'ProductOption', 'OptionValue', 'ProductAssignCrossproduct', 'Cart', 'CartOption','FooterBlock','ProductShippingValue','CatalogProduct', 'Wishlist','ProductAssignNewOption');
	public $paginate = array();

	public function beforeFilter() 
	{
		parent::beforeFilter();
		$this->Auth->allow('home','products','productssl','productssr','ajaxproductlist', 'ajaxproductlist3c', 'ajaxaddtowishlist', 'productcount', 'ajaxproductpopup','productdetails','ajaxReview','ajaxminicart','showminicart','ajaxremoveminicart','showcorexminicart','search','admin_getListProduct','admin_getListProduct1','mycatalog','catalogdetail','admin_priceupdate','admin_sequenceupdate','admin_ajaxaddoption','ajaxpricechange');
	...
	public function admin_index()
	{
		...
		$categories = $this->ProductCategory->generateTreeList(array('ProductCategory.is_del'=> '0'),null, null, '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;');
		...
		if($this->request->is("post"))
		{
			...
			$data['Product']['sequence']='';
			
			$likekeyArr = array('product_name','product_sku','product_categoryid');
	...
	public function admin_manage($id=NULL){	
		...
		if($this->request->is('post')){	
			if($this->Product->validates()){
				...
				$this->ProductAssignNewOption->deleteAll(array('ProductAssignNewOption.product_id' => $id));	
					
				if(!empty($data['ProductAssignNewOption']['option_value_id']))
					{
					
				//	pr($data['ProductShippingValue']);
						foreach($data['ProductAssignNewOption']['option_value_id'] as $key1=>$value1)
						{
							$i=0;
							//echo $key1;
						...
						//	echo $value1;
							...
							{
								if($value2!='' && $data['ProductAssignNewOption']['Quantity'][$key1][$i]!='' && $data['ProductAssignNewOption']['operation_value'][$key1][$i]!='') {
								//pr($value2);
								$this->ProductAssignNewOption->create();
								$productshippingvalueArr['ProductAssignNewOption']['product_id']=$this->Product->id;
								$productshippingvalueArr['ProductAssignNewOption']['option_id']=$key1;
								$productshippingvalueArr['ProductAssignNewOption']['option_value_id']=$value2;
								
								$productshippingvalueArr['ProductAssignNewOption']['Quantity']=$data['ProductAssignNewOption']['Quantity'][$key1][$i];
								$productshippingvalueArr['ProductAssignNewOption']['operation']=$data['ProductAssignNewOption']['operation'][$key1][$i];
								$productshippingvalueArr['ProductAssignNewOption']['operation_value']=$data['ProductAssignNewOption']['operation_value'][$key1][$i];
								$this->ProductAssignNewOption->save($productshippingvalueArr);
								}
								$i++;
							}
		...
		if($id != NULL || $id !== '')
		{
			$this->Product->bindModel(array(
								'hasMany' => array(
									...
									'ProductAssignNewOption' => array(
										'className'    => 'ProductAssignNewOption',
										'foreignKey'   => 'product_id'
									)
								)
							));
			...
			if(!empty($data['ProductAssignNewOption'])){
				foreach($data['ProductAssignNewOption'] as $key=>$ProductShippingValues)
					{ 
					
						if (in_array($ProductShippingValues['option_id'],$optionidArr))
						  {
							
						  }
						else
						  {
							array_push($optionidArr,$ProductShippingValues['option_id']);
						  }
					}
			}
			...
			$this->set('data', $data);
			$this->set('optionidArr', $optionidArr);
		} else {
			$prdAssignIDString = "";
	...
	private function mouldOptions($product_id){
		...
		$this->ProductAssignNewOption->bindModel(array(
				'belongsTo'=>array(
					'ProductOption'=>array(
		...
		$options = $this->ProductAssignNewOption->find('all', array(
				'conditions'=>array(
					'ProductAssignNewOption.product_id'=>$product_id
				)
			)
		);
	...
	public function productdetails($slug=NULL, $catalogid=NULL)
	{
		...
		$this->set(compact('product','productid','options','reviews', 'slug','catalogid','crossProductlist')); 
	}
	...
	public function ajaxminicart(){
		...
		if($this->request->is('post')){
			...
			$productid=$reqdata['Cart']['product_id'];
			$cartquantity=$reqdata['Cart']['quantity'];
			$sessionid=$reqdata['Cart']['session_id'];
			$cartdetails = $this->Cart->find('all', array(
					'conditions'=>array(
			...
			$productoriginalqu=$this->Product->find('all', array(
					'conditions'=>array(
			...
			$adminproductquntity=$productoriginalqu[0]['Product']['product_quantity'];
			$productquntity=0;
			if(!empty($cartdetails))
			{
				$productquntity=$cartdetails[0]['Cart']['quantity'];
			}
			$compairproductquntity=$productquntity+$cartquantity;
			$flag=0;
			if($compairproductquntity<=$adminproductquntity){
	...
	public function search($productname)
	{
		...
		'conditions' => array('Product.isdel'=>'0','Product.product_status'=>'Y','Product.product_name LIKE'=>'%'.$productname.'%'),
				//'order'=>array('Product.id' => 'DESC')
	...
	public function admin_ajaxaddoption(){
		$this->layout = 'ajax';
		$this->loadModel('OptionValue');
		if($this->request->is('post')){
			 $id = $this->request->data['id'];
			 $length = $this->request->data['length'];
			 $latestval = $this->request->data['latestval'];
			 $optionvalueArray=$this->OptionValue->find('list',array(
										'conditions'=>array('OptionValue.option_id'=>$id),
										'fields'=>array('option_value_name')
										));
			$optionvaluecountArray=$this->OptionValue->find('count',array(
										'conditions'=>array('OptionValue.option_id'=>$id)
										));
										
		}
		$this->set(compact('optionvalueArray','id','length','latestval','optionvaluecountArray')); 
	}
	
	public function ajaxpricechange(){
		$this->layout = 'ajax';
		$this->autoRender=false;
		$this->loadModel('ProductAssignNewOption');
		if($this->request->is('post')){
				$actualprice = $this->request->data['actualprice'];
				$actualproductprice = $this->request->data['actualproductprice'];
				if(!empty($this->request->data['idArr']))
				{
				...
				$optionid = $this->request->data['optionid'];
				
				$productid = $this->request->data['productid'];
				if(!empty($idArr))
				{
				 foreach($idArr as $key=>$valuearr)
				 { 
					   $pricedata = $this->ProductAssignNewOption->findByProductIdAndOptionValueId($productid,$valuearr);
					   if($pricedata['ProductAssignNewOption']['operation']=='1')
					   {
							 $actualproductprice=($actualproductprice +  $pricedata['ProductAssignNewOption']['operation_value']);
					   }
					   else 
					   {
						 $actualproductprice=($actualproductprice -  $pricedata['ProductAssignNewOption']['operation_value']);
				 	...
				 	echo  number_format($actualproductprice,2);
				exit();
		}
	}
