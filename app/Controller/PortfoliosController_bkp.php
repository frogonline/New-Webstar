...
class PortfoliosController extends AppController {
	...
	public $uses = array('Portfolio', 'PortfolioContent', 'Style', 'Shortcode', 'PageTemplateRowsColumn');
	...
	public function admin_manage($id=NULL){
		...
		if($this->request->is("post")){
			...
			if($data['Portfolio']['status']=='Y')
			{
				...
				if(empty($get_data))
				{
					$styleData = $this->Style->findByGroupId(31);
					//pr($styleData); exit;
	...
	public function admin_ajaxcopyitem(){
		...
		if($this->request->is('post')){
			...
			if(!empty($data)){
				...
				$orgWidget = $this->Portfolio->findById($data['widgetId']);
				...
				if(!empty($newId)){
					...
					$shortcode = $this->Shortcode->findByGroupIdAndWidgetId(31, $data['widgetId']);
					if(!empty($shortcode)){
					...
					if(!empty($data['colId'])){
						...
						$tpldata['PageTemplateRowsColumn']['shortcode'] = '[Portfolio-'.$newId.']';
