...
<?php echo $this->Form->input('id',array("type"=>"hidden","id"=>"productid","label"=>false,"value"=> $productid)); ?>
<?php echo $this->Form->input('previousval',array("type"=>"hidden","id"=>"previousval","label"=>false,"value"=>'')); ?>
	...
	<div class="container">
		...
		<?php //echo $this->Layout->CorexproductBreadcrumb($slug); ?>
			...
			<div class="col-md-6 main-el">
				<h3 class="ecombox_pname"><?php echo (!empty($product))?$product['Product']['product_name']:''; ?></h3>
				<h5 class="main-text-color">
				<?php echo $this->Form->input('actualproductprice',array("type"=>"hidden","id"=>"actualproductprice","label"=>false,"value"=>number_format($this->Layout->actualprice($product['Product']['id'],$catalogid),2) )); ?>
				
				
				<strong><span class="ecombox_pprice"><?php echo CURRENCY; ?></span><span id="actualprice" class="ecombox_pprice"><?php echo number_format($this->Layout->actualprice($product['Product']['id'],$catalogid),2); ?></span></strong>
				
				<?php if($product['Product']['product_discount']!=0){ ?>
				<em class="ecombox_pprice"><?php echo CURRENCY; ?><span class="main-price ecombox_pprice"><?php echo (!empty($product))?number_format($this->Layout->mainprice($product['Product']['id'],$catalogid),2):'0.00'; ?></span></em>
				<?php } ?>
				</h5>
				<h5 class="main-text-color stock_txt_clr">
				<?php 
				if(!empty($product)){
				...
				<span class="product_describe_txt_clr">
				<p class="product_describe_txt_clr"><?php echo (!empty($product))?$product['Product']['product_shortdesc']:''; ?></p></span>
				...
				?>
					<div class="row">
						...
						<div class="col-md-4 product_describe_txt_clr">
							<select name="data[CartOption][option_value][<?php echo $optionVal['id']; ?>]" class="form-control variance product_describe_txt_clr" id="<?php echo $optionVal['id']; ?>">
				...
				<div class="row controls">
					<div class="col-md-2">
						<label class="control-label product_describe_txt_clr">Qty:</label>
					</div>
					<?php if($ThemeSettingheadertype=='V') {?>
					<div class="col-md-5">
					<?php }else{ ?>
					...
					<?php
						echo $this->Html->link('<div class="oparetor">-</div>', 'javascript:void(0);', array('class'=>'button grey solid ctrl product_describe_txt_clr', 'escape'=>false, 'onclick'=>'fnDecrese(this);'));
						...
						echo $this->Html->link('<div class="oparetor">+</div>', 'javascript:void(0);', array('class'=>'button grey solid ctrl product_describe_txt_clr', 'escape'=>false, 'onclick'=>'fnIncrese(this);'));
					?>
					</div>
				</div>
				<?php
				...
				echo $this->Form->button('<div class="over">Add to cart</div>', array('type' => 'submit','class'=>'button solid blue md add addcart_btn_backgound cartaddcls ecombox_addtocart', 'escape'=>false));
				echo "</div>";

				echo "<div class='col-sm-6'>";
				echo $this->Layout->wishlistLinkproductdetails($product['Product']['id']);
				echo "</div>";
				echo "</div>";
				...
				<div id="d-tabs" class="tab def review_txt_clr">
					<ul>
						<li><a href="#d-tabs-1"><h6 class="review_txt_clr">Description </h6></a></li>
						<li><a href="#d-tabs-2"><h6 class="review_txt_clr">Information</h6></a></li>
						<li><a href="#d-tabs-3"><h6 class="review_txt_clr">Reviews (<?php echo count($reviews); ?>)</h6></a></li>
					</ul>
					<div id="d-tabs-1">
					  ...
					  <?php  
					   ...
					   <?php
						...
						<div class="form-group">
							...
							<?php echo $this->Form->input('rating',array('id'=>"backing5",  'class'=>"form-control",'value'=>"4",'div'=>false, 'label'=>false,'type'=>'hidden')); ?>
							<span class="star-cb-group">
							  <input type="radio" class="rat" id="rating-5" name="rating" value="5" /><label for="rating-5">5</label>
							  <input type="radio" id="rating-4" class="rat" name="rating" value="4" checked="checked" /><label for="rating-4">4</label>
							  <input type="radio" class="rat" id="rating-3" name="rating" value="3" /><label for="rating-3" >3</label>
							  <input type="radio" class="rat" id="rating-2" name="rating" value="2" /><label for="rating-2">2</label>
							  <input type="radio" id="rating-1" class="rat" name="rating" value="1" /><label for="rating-1">1</label>
						...
						<div class="padding-top-20">
							...
							<?php echo $this->Form->button('Send', array('type' => 'submit','value'=>'Send','class'=>'btn btn-primary send_btn_backgound_clr'));?>
						</div>
...
<div class="container">     
	<div class="row">
		<div class="col-md-12 main-el hidden-xs">
			...
			<h3 class="main-text-color">YOU MAY ALSO LIKE THESE</h3>
			</div> 
...
$(".variance").change(function(){
        var actualproductprice=$('#actualproductprice').val();
		var actualprice = $('#actualprice').text();
		var optionvalue = $(this).val();
		var optionid = $(this).attr('id');
		var previousval = $('#previousval').val()
		...
		var idArr = new Array();
				...
				//alert(idArr);
				 ...
				 $.ajax({
					type:'POST',
					url:'<?php echo $this->Html->url(array('controller'=>'Products', 'action'=>'ajaxpricechange', 'full_base'=>true)); ?>',
					data:{actualproductprice:actualproductprice,actualprice:actualprice,optionvalue:optionvalue,productid:productid,optionid:optionid,previousval:previousval,idArr:idArr},
						success:function(result){
						var splitResult=result.split("/");
						$("#actualprice").html(splitResult[0]);
						$("#CartUnitPrice").val(splitResult[0]);
					
						$("#previousval").val(splitResult[1]);
...
function wishlistproductlog(el){
