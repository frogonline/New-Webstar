...
e.preventDefault();
	...
	$('.contact_usform').submit(function(e){
		...
		$('#contacts-email').css('border', 'solid 2px #F00');
		$('#phone').css('border', 'solid 2px #F00');
		$('#contacts-message').css('border', 'solid 2px #F00');
		return false;
		}else if(name == ''){
		$('#name').css('border', 'solid 2px #F00');
		return false;
		...
		$('#contact_msg').html('<p style="color:red" class="alert alert-danger">reCaptcha not verified</p>');
		return false;
		}else{
				$.ajax({
					type:'POST',
	...
	$('.contact_usform1').submit(function(e){
		...
		return false;
				...
				$.ajax({
					...
					success:function(result){
						if(result == 4){
							//$('#contact_msg').html('<p style="color:red" class="alert alert-success">Form & Resource details has been sent.</p>');
							window.location.href="<?php echo SITE_URL; ?>thank-you";
							return false;
						} else if(result == 3) {
							$('#contact_msg').html('<p class="alert alert-danger">Failed to send mail.</p>');
...
$('#contacts-email').blur(function () {
   var contacts_email = $("#contacts-email").val();
   if (contacts_email != "") {
        $("#contacts-email").css('border', 'none');
    } 
...
$('#contacts-message').blur(function () {
   var contacts_message = $("#contacts-message").val();
   if (contacts_message != "") {
        $("#contacts-message").css('border', 'none');
    } 
