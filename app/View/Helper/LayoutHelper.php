...
class LayoutHelper extends AppHelper {
	...
	public function productlist($conditionsArr=NULL, $order=NULL, $limit=0, $offset=0){
		...
		$orderArr = array();
		 ...
		 if(!empty($order)){
			...
			$orderArr = array('Product.sequence' => 'ASC');
		} 
		...
		$conditions['Product.product_status'] = 'Y'; 
	...
	*/
		...
		<div class="col-sm-6 col-md-9 col-xs-12">
			...
			?>
				<div class="item">
					...
					<?php
					$product_name_lal=(strlen($item['Product']['product_name'])>20)?substr($item['Product']['product_name'],0,20).'...':$item['Product']['product_name'];

					?>
						<p><?php echo $this->Html->link($product_name_lal, SITE_URL.$item['Product']['product_slug']); ?></p>
						
						
						
	<p><?php echo CURRENCY.''.number_format($item['Cart']['unit_price'], 2); ?>/Quantity:<?php echo $item['Cart']['quantity']; ?></p>
		...
		<div class="col-sm-6 col-md-3 col-xs-12 cart">
			...
			echo $this->Html->link('<div class="over addwishlist_btn_backgound">Proceed to Checkout</div>', SITE_URL.'checkout', array('class'=>'button solid md blue addwishlist_btn_txt_clr', 'escape'=>false));
			?>
		...
		if(!empty($product_id)){
			if($loginStatus){
				...
				if(in_array($product_id, $wishlistProducts)){
					$str .= $this->Html->link('<i class="fa fa-heart"></i>', 'javascript:void(0);', array('escape'=>false, 'class'=>'customize_mergleft addtowishlist','title'=>'Remove from wishlist', 'id'=>'prd-wishlst-'.$uniqueid.$product_id, 'onclick'=>'return wishlist(this);', 'data-product_id'=>$product_id, 'data-member_id'=>$member_id, 'data-url'=>$this->Html->url(array('controller'=>'Products', 'action'=>'ajaxaddtowishlist', 'full_base'=>true))));
				} else {
					$str .= $this->Html->link('<i class="fa fa-heart-o"></i>', 'javascript:void(0);', array('escape'=>false, 'class'=>'customize_mergleft addtowishlist','title'=>'Add to wishlist', 'id'=>'prd-wishlst-'.$uniqueid.$product_id, 'onclick'=>'return wishlist(this);', 'data-product_id'=>$product_id, 'data-member_id'=>$member_id, 'data-url'=>$this->Html->url(array('controller'=>'Products', 'action'=>'ajaxaddtowishlist', 'full_base'=>true))));
				}
			}
		}
		return $str;
	...
	public function pagecrumb($type, $title, $slug=NULL, $Parentpagename, $opt=NULL){
		...
		if(!empty($setting)){
			if($setting['ThemeSetting']['header_type']=='H'){
				if(in_array($type, $typeArr)){
					if($type == 'product'){
						if(!empty($opt)){
	$breadcrumb = $breadcrumb = $this->Html->link('HOME', SITE_URL,array('class'=>'bedcrumbright_control')) . ' / '.strtoupper($title);
	
						} else {
							$breadcrumb = $this->productBreadcrumb($slug,array('class'=>'bedcrumbright_control'));
						}
					} else if($type == 'single-post') {
						$breadcrumb = $this->Html->link('HOME', SITE_URL,array('class'=>'bedcrumbright_control')).' / '.$this->Html->link('BLOG', SITE_URL.'blog/',array('class'=>'bedcrumbright_control')).' / '.strtoupper($title);
					} else {
						if(empty($Parentpagename)){
						$breadcrumb = $this->Html->link('HOME', SITE_URL,array('class'=>'bedcrumbright_control')).' / '.strtoupper($title);
						}else {
							
							$Parentpagename=str_replace("%20"," ",$Parentpagename);
							$breadcrumb = $this->Html->link('HOME', SITE_URL,array('class'=>'bedcrumbright_control')).' / '.strtoupper($Parentpagename).' / '.strtoupper($title);
					...
					$str .= '<div class="pagecrumbs bedcrumb_backgound">
								<div class="container">
									<div class="row">
										<div class="col-xs-4">
											<h5 class="medium titlecrumbs bedcrumbleft_control" id="breadcrumbtitle">'.$title.'</h5>
										</div>
										<div class="col-xs-8">
											<div class="location pull-right bedcrumbright_control">
								<span class="medium positioncrumbs bedcrumbright_control">YOU ARE HERE: </span> '.$breadcrumb.'
											</div>
										</div>
									</div>
								</div>
	...
	public function optionvaluesname($id) {
        App::import('Model', 'OptionValue');
		$optionvalue = new OptionValue();
		
		$optionvalues = $optionvalue->find('list',array(
	...
	public function wishlistLinkproductdetails($product_id, $uniqueid=NULL){
		App::import('Model', 'Wishlist');
		$wishlistObj = new Wishlist();
		...
		if(!empty($product_id)){
			if($loginStatus){
				...
				if(in_array($product_id, $wishlistProducts)){
		$str .= $this->Form->button('<div class="over"><i class="fa fa-heart"></i>REMOVE WISHLIST</div>',  array('type' => 'submit','escape'=>false, 'class'=>'customize_mergbutton button solid md addtowishlist red addwishlist_btn_backgound addwishlist_btn_txt_clr','title'=>'Remove from wishlist', 'id'=>'prd-wishlst-'.$uniqueid.$product_id, 'onclick'=>'return wishlistproduct(this);', 'data-product_id'=>$product_id, 'data-member_id'=>$member_id, 'data-url'=>$this->Html->url(array('controller'=>'Products', 'action'=>'ajaxaddtowishlist', 'full_base'=>true))));
					
					
					
					
				} else {
	$str .= $this->Form->button('<div class="over cartaddcls"><i class="fa fa-heart-o cartaddcls"></i>ADD WISHLIST</div>', array('type' => 'submit','escape'=>false, 'class'=>'customize_mergbutton button solid md addtowishlist green addwishlist_btn_backgound addwishlist_btn_txt_clr cartaddcls','title'=>'Add to wishlist', 'id'=>'prd-wishlst-'.$uniqueid.$product_id, 'onclick'=>'return wishlistproduct(this);', 'data-product_id'=>$product_id, 'data-member_id'=>$member_id, 'data-url'=>$this->Html->url(array('controller'=>'Products', 'action'=>'ajaxaddtowishlist', 'full_base'=>true))));
				}
			}
			else{
			
$str .= $this->Form->button('<div class="over cartaddcls"><i class="fa fa-heart-o cartaddcls"></i>ADD WISHLIST</div>', array('type' => 'button','escape'=>false, 'class'=>'customize_mergbutton button solid md green addwishlist_btn_backgound addwishlist_btn_txt_clr cartaddcls','title'=>'Add to wishlist','onclick'=>'return wishlistproductlog(this);'));
	...
	public function rating($product_id){
		...
		App::import('Model', 'ProductReview');
		$Productreview = new ProductReview();
		
		
		
		$options = $Productreview->find('all', array(
				'conditions'=>array(
					'ProductReview.product_id'=>$product_id,
					'ProductReview.status'=>'Y'
				)
			)
		);
			...
			foreach ($options as $item){
				$rating=($rating + $item['ProductReview']['rating']);
				$i++;
			}
			$ratingpoint=($rating)/$i;
			return $ratingpoint;
		}
