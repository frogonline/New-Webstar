...
class MenuitemMastersHelper extends AppHelper {
	...
	private function create_menu($data, $arr, $ent=0){
		...
		if(count($data) > 0){
			...
			foreach($data as $item){
				$str.=(array_key_exists('child', $item['MenuitemMaster']))?'<li class="'.$arr['hasChildli_class'].'">':'<li>';
				$str.='<a href="'.$item['MenuitemMaster']['page_url'].'">'.$this->Html->tag($arr['item_wrap']).$item['MenuitemMaster']['page_title'].'</a>';
