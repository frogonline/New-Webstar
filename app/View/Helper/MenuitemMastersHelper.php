...
class MenuitemMastersHelper extends AppHelper {
	...
	private function create_menu($data, $arr, $ent=0){
		...
		if(count($data) > 0){
			$str.=($ent==0)?'<ul class="'.$arr['menu_class'].' menu_control" id="'.$arr['menu_id'].'">':'<ul class="'.$arr['submenu_class'].'">';
			if(($arr['mobile_menu']=='yes') && $ent==0){
				$str.='<li class="main alt-bg-color menu_control">
			...
			foreach($data as $item){
				...
				if(array_key_exists('parent_active', $item['MenuitemMaster'])){
					$str.=(array_key_exists('child', $item['MenuitemMaster']))?'<li class="'.$arr['hasChildli_class'].' active menu_control">':'<li class="'.$arr['item_class'].' active menu_control">';
				} else {
					if($currentUrl == $itemurl) {
					$str.=(array_key_exists('child', $item['MenuitemMaster']))?'<li class="'.$arr['hasChildli_class'].' active menu_control">':'<li class="'.$arr['item_class'].' active menu_control">';
					} else {
						$str.=(array_key_exists('child', $item['MenuitemMaster']))?'<li class="'.$arr['hasChildli_class'].' menu_control">':'<li class="'.$arr['item_class'].' menu_control">';
					}
				}
				
				
				$str.='<a href="'.$item['MenuitemMaster']['page_url'].'" class="menu_control boldc">'.$this->Html->tag($arr['before_item'], '', array('class' => $arr['before_item_class'])).$this->Html->tag($arr['item_wrap'],$item['MenuitemMaster']['page_title'], array('class'=>$arr['item_wrap_class'])).$this->Html->tag($arr['after_item'], '', array('class' => $arr['after_item_class'])).'</a>';
				
				if(array_key_exists('child', $item['MenuitemMaster'])){
					$n=count($item);
					$ent++;
					$str.=$this->create_submenu($item['MenuitemMaster']['child'], $arr, $ent);
					$ent--;
				}
				else{
					$str.='</li>';
	...
	private function create_submenu($data, $arr, $ent=0){
		...
		if(count($data) > 0){
			...
			foreach($data as $item){
				...
				$arr['item_wrap']='';
				$arr['item_wrap_class']='';
	$str.='<a href="'.$item['MenuitemMaster']['page_url'].'" class="sunmenu_control">'.$this->Html->tag($arr['before_item'], '', array('class' => $arr['before_item_class'])).$this->Html->tag($arr['item_wrap'],$item['MenuitemMaster']['page_title'], array('class'=>$arr['item_wrap_class'])).$this->Html->tag($arr['after_item'], '', array('class' => $arr['after_item_class'])).'</a>';
